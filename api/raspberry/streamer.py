...
# Configuration
VEHICLE_ID = os.getenv('VEHICLE_ID', 'KAA_123X')
API_URL = os.getenv('API_URL', 'http://your-server.com:8000')
MQTT_BROKER = os.getenv('MQTT_BROKER', '172.168.100.103')
...
class VehicleDataCollector:
    ...
    def send_at(self, command, delay=0.5):
    ...
    def connect_gps(self):
        ...
        try:
            ...
            logger.info("Checking GPS module...")
            response = self.send_at("AT")
            if "OK" in response or "AT" in response:
                logger.info("GPS module responding")

                # Turn GPS on
                logger.info("🛰️ Enabling GPS...")
                response = self.send_at("AT+CGPS=1")
    ...
    def read_obd_data(self):
        ...
        try:
            data = {
                ...
                "oil_pressure": self._safe_query(obd.commands.BAROMETRIC_PRESSURE),
                "battery_voltage_v" : self._safe_query(obd.commands.ELM_VOLTAGE),
                "fuel_rate":self._safe_query(obd.commands.FUEL_RATE),
                "fuel_status": self._safe_query(obd.commands.FUEL_STATUS),
                "engine_runtime": self._safe_query(obd.commands.RUN_TIME),
                "engine_temp_c": self._safe_query(obd.commands.OIL_TEMP),
            }
            logger.info(f"OBD data read: {data}")
            return data
    ...
    def read_gps_data(self):
        """Read GPS data using AT commands (polling mode)"""
        if not self.gps_serial:
            return
        ...
        try:
            ...
            if "+CGPSINFO:" in response:
                # Example format: +CGPSINFO: 0115.735314,S,03645.993231,E,141025,114452.0,1812.7,0.0,45.2
                try:
                    # Extract the data part after +CGPSINFO:
                    data_part = response.split("+CGPSINFO:")[1].split("\r\n")[0].strip()
                    parts = [p.strip() for p in data_part.split(",")]

                    # Check if we have enough parts and valid data
                    if len(parts) >= 4:
                        lat_raw = parts[0]
                        lat_dir = parts[1]
                        lon_raw = parts[2]
                        ...
                        if lat_raw and lon_raw and lat_raw != "" and lon_raw != "":
                            ...
                            logger.debug(f"📍 Latitude: {lat_deg:.6f}, Longitude: {lon_deg:.6f}")
                        else:
                            # Empty coordinates means no GPS fix yet
                            if self.gps_fix:
                    ...
                    logger.debug(f"GPS parse error: {e} - Response: {response.strip()}")
                    self.gps_fix = False
            else:
                # No +CGPSINFO in response (probably just "OK")
                logger.debug("❌ No GPS fix yet...")
                self.gps_fix = False
    ...
    def combine_data(self):
        ...
        telemetry = {
            ...
            'engine': {
                ...
                'oil_pressure': self.current_obd_data.get('oil_pressure'),
                'battery_voltage_v': self.current_obd_data.get('battery_voltage_v'),
                'fuel_status': self.current_obd_data.get('fuel_status'),
                'engine_runtime': self.current_obd_data.get('engine_runtime'),
                'engine_temp_c': self.current_obd_data.get('engine_temp_c')
            }
        }
